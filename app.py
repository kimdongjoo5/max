...
<style>
    ...
    /* 퀵 링크 카드 */
    .link-box { text-align: center; padding: 8px; background: #fdfdfd; border-radius: 8px; border: 1px solid #eee; transition: 0.2s; text-decoration: none; color: black; display: block; }
    .link-box:hover { background: #f1f7ff; border-color: #3498db; }
    ...
    /* 모바일 환경(화면이 좁을 때) 특별 적용 */
    @media (max-width: 768px) {
        .main-title { font-size: 1.4rem !important; }
        .link-box { padding: 4px; }
        .link-icon { font-size: 1.2rem !important; }
        .link-text { font-size: 0.65rem !important; }
...
for i, l in enumerate(links):
    with cols[i]:
        st.markdown(f'<a href="{l["url"]}" target="_blank" class="link-box"><div class="link-icon" style="font-size:1.5rem;">{l["i"]}</div><div class="link-text" style="font-size:0.75rem; font-weight:bold;">{l["n"]}</div></a>', unsafe_allow_html=True)

st.write("")

# --- 로그인 및 날짜 ---
st.error("🚨 [필수 확인] 아래에 닉네임을 입력해야만 파티 예약(빈자리 터치) 및 취소가 가능합니다!")
        ...
        if d_list:
            ...
            for idx, p in enumerate(d_list):
                with p_cols[idx % 3]:
                    ...
                    # 모바일 최적화 초소형 버튼 (빈자리 터치 효과)
                    if is_logged:
                        if u_name in mems:
                            if st.button("❌ 내 자리 빼기 (나가기)", key=f"out_{p_id}", use_container_width=True):
                                p["members"].remove(u_name)
                                if not p["members"]: d_list.remove(p)
                                save_json(PARTY_DB_FILE, parties_db); st.rerun()
                        elif len(mems) < cap:
                            if st.button("➕ 빈자리 채우기 (참여)", key=f"in_{p_id}", type="primary", use_container_width=True):
                                p["members"].append(u_name); save_json(PARTY_DB_FILE, parties_db); st.rerun()
                        else:
                            st.button("🔒 인원 마감", key=f"f_{p_id}", disabled=True, use_container_width=True)
                    else:
                        st.button("닉네임 입력 후 참여 가능", key=f"d_{p_id}", disabled=True, use_container_width=True)
            st.write("")
            ...
            if st.form_submit_button("🚀 파티 개설하기", use_container_width=True):
                ...
                def get_h(ts):
                    try:
                        ...
                        hr = int(h.replace("시",""))
                        return hr + 12 if a=="오후" and hr!=12 else (0 if a=="오전" and hr==12 else hr)
                    except: return 0
